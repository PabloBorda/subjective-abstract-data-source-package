        ...
        if os.path.exists(installed_plugins_dir):
            for plugin_dir in os.listdir(installed_plugins_dir):
                ...
                if os.path.isdir(plugin_path):
                    # Look for DataSource.py files in this plugin directory
                    for file in os.listdir(plugin_path):
                        if file.endswith("DataSource.py"):
                                    ...
                                    if f"class {ds_class_name}" in file_content:
                                        ...
                                        try:
                                            ...
                                            if hasattr(module, ds_class_name):
                                                ds_class = getattr(module, ds_class_name)
                                                BBLogger.log(f"Successfully imported {ds_class_name} from {plugin_dir}/{file}")
                                                return ds_class
                                        except Exception as import_err:
                                            BBLogger.log(f"Failed to import {module_name}: {import_err}")
                                            raise import_err
                                    else:
                                        BBLogger.log(f"{ds_class_name} not found in {file}")
